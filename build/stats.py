...
bucks = [b for b in BUCKETS if any(k[1]==b for k in sk_yb)]
...
data = dict(
  ...
  nights_by_bucket={b:nights_b[b] for b in BUCKETS if nights_b[b]},
  years=year_rows,
...
print("\nBy bucket:"); [print(f"  {b:<26}{nights_b[b]:>4}n") for b in BUCKETS if nights_b[b]]
...
print(f"\nSankey nodes {len(nodes)} links {len(links)} | map points {len(map_places)} | seq {len(seq)}")
